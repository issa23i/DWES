...
// si viene de una redirecdion
if(isset($_REQUEST['redirigido'])){
    $mensaje = 'Realiza el login';
}
...
// LOGIN
    if (isset($_POST['enviar_login'])) {
        ...
        $clave_usuario = md5($_POST['Clave']);
        $query_login = "SELECT * FROM `usuarios` WHERE Nombre = :nombre_usuario AND Clave =  :clave_usuario";
...
<?php if (isset($nombre_usuario) && isset($clave_usuario)) : ?>
    ...
    <form action="<?= $_SERVER['PHP_SELF'] ?>" method="POST">
        ...
        <input type="password" name="Nombre_usuario" id="Nombre_usuario" class="form-control">
